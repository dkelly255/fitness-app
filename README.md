...
The key tests below are part of the manual testing procedures I have followed to prove out the functionality of the primary workflows available to shoppers, ommunity members, and store owners within the application:
  ...
  **Authorisation & authentication Testing:**
  ...
  ------------- |------------- | ------------- | ------------ | ------------- | ------------- 
  1 | Register an account | When clicking the `Sign Up` option, a user should be presented with the sign up form. Upon population of the required fields within the form, the user will be sent a confirmation email asking them to confirm their email to receive a newly created account. | User account created as expected | Pass | [Screenshot 1](readme/testing/test1-signup.png) [Screenshot 2](readme/testing/test1-signup-confirm.png)
  2 | Confirmation Email | After entering their details on the Registration page, the user should receive a confirmation email requesting them to confirm their email address in order to activate their account | Functionality operates as expected | Pass | [Screenshot 1](readme/testing/test2-signup-email.png) [Screenshot 2](readme/testing/test2-signup-final.png)
  3 | Log in | Upon clicking the `Login` option from the navigation bar, the `Log In` form should display to the user, prompting a username & password. Upon entering these details, a user should be logged into the site| Log in option & form functions as expected | Pass | [Screenshot](readme/testing/test2-signin.png)
  4 | Log out | Upon clicking the `Logout` option from the navigation bar, a user should be promted to confirm they wish to logout, and upon confirmation, they should be logged out of the site | Functionality operates as expected|Pass | [Screenshot](readme/testing/test2-signin.png)
